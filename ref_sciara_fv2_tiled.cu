...
// ----------------------------------------------------------------------------
// CUDA Kernels - Version 2: TILED with Shared Memory (No Halo)
// ----------------------------------------------------------------------------
...
// Simple kernels kept from V1 (no tiling benefit)
...
// VERSION 2: TILED computeOutflows with Shared Memory (No Halo)
...
{
    // Shared memory for tile (16x16)
    __shared__ double s_Sz[16][16];
    __shared__ double s_Sh[16][16];
    __shared__ double s_ST[16][16];
    ...
    for (int k = 0; k < MOORE_NEIGHBORS; k++) {
        ...
        // Check if neighbor is in shared memory tile (NO HALO)
        int ni = gi + Xi[k];
        int nj = gj + Xj[k];
        int nti = ti + Xi[k];
        int ntj = tj + Xj[k];
        
        // Use shared memory if neighbor is within tile, otherwise global
        if (nti >= 0 && nti < 16 && ntj >= 0 && ntj < 16) {
            // Neighbor in shared memory
            sz = s_Sz[nti][ntj];
            h[k] = s_Sh[nti][ntj];
        } else {
    ...
    // Rest of computation (same as V1)
    H[0] = z[0];
    theta[0] = 0;
    eliminated[0] = false;
...
// VERSION 2: TILED massBalance with Shared Memory
__global__ void massBalance_tiled_kernel(
    ...
    double *Mf)
{
    // Shared memory for Mf tile (more efficient for multiple neighbor reads)
    __shared__ double s_ST[16][16];
    ...
    for (int n = 1; n < MOORE_NEIGHBORS; n++) {
        ...
        // Use shared memory if available, otherwise global
        if (nti >= 0 && nti < 16 && ntj >= 0 && ntj < 16) {
            neigh_t = s_ST[nti][ntj];
        } else {
...
// Keep V1 version for comparison (simple, no tiling needed)
...
void computeOutflows_tiled_cuda(Sciara *sciara, dim3 grid, dim3 block)
{
    int *d_Xi, *d_Xj;
    ...
    computeOutflows_tiled_kernel<<<grid, block>>>(
        sciara->domain->rows,
        sciara->domain->cols,
        d_Xi, d_Xj,
        sciara->substates->Sz,
...
void massBalance_tiled_cuda(Sciara *sciara, dim3 grid, dim3 block)
{
    int *d_Xi, *d_Xj;
    cudaMalloc(&d_Xi, sizeof(int) * MOORE_NEIGHBORS);
    cudaMalloc(&d_Xj, sizeof(int) * MOORE_NEIGHBORS);
    ...
    massBalance_tiled_kernel<<<grid, block>>>(
        sciara->domain->rows,
        sciara->domain->cols,
        d_Xi, d_Xj,
        sciara->substates->Sh,
...
int main(int argc, char **argv)
{
           ...
           (total_current_lava == -1 || total_current_lava > thickness_threshold))
    {
        ...
        // Apply the TILED computeOutflows kernel
        computeOutflows_tiled_cuda(sciara, grid, block);

        // Apply the TILED massBalance kernel
        massBalance_tiled_cuda(sciara, grid, block);
        cudaMemcpy(sciara->substates->Sh, sciara->substates->Sh_next, 
                   sizeof(double) * sciara->domain->rows * sciara->domain->cols, 
                   cudaMemcpyDeviceToDevice);
        cudaMemcpy(sciara->substates->ST, sciara->substates->ST_next, 
