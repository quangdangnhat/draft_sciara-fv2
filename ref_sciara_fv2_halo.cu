...
// ----------------------------------------------------------------------------
// Tiling parameters for V3 (with halo)
// ----------------------------------------------------------------------------
#define TILE_SIZE 16
#define HALO_SIZE 1
#define SHARED_SIZE (TILE_SIZE + 2 * HALO_SIZE)  // 18x18
...
// ----------------------------------------------------------------------------
// CUDA Kernels - Version 3: TILED with Halo Cells
// ----------------------------------------------------------------------------
...
// Simple kernels kept from V1/V2 (no changes)
...
// VERSION 3: TILED computeOutflows with HALO CELLS
__global__ void computeOutflows_halo_kernel(
    int r, int c,
    int *Xi, int *Xj,
    double *Sz, double *Sh, double *ST, double *Mf,
    double Pc, double _a, double _b, double _c, double _d)
{
    // Shared memory with halo: 18x18 (16 + 2 halo)
    __shared__ double s_Sz[SHARED_SIZE][SHARED_SIZE];
    __shared__ double s_Sh[SHARED_SIZE][SHARED_SIZE];
    __shared__ double s_ST[SHARED_SIZE][SHARED_SIZE];
    ...
    int si = threadIdx.y + HALO_SIZE;  // +1 offset
    int sj = threadIdx.x + HALO_SIZE;  // +1 offset
    
    int ti = threadIdx.y;
    int tj = threadIdx.x;
    
    // Load center tile (16x16 threads load 16x16 data)
    if (gi < r && gj < c) {
        s_Sz[si][sj] = GET(Sz, c, gi, gj);
        s_Sh[si][sj] = GET(Sh, c, gi, gj);
        s_ST[si][sj] = GET(ST, c, gi, gj);
    } else {
        s_Sz[si][sj] = 0.0;
        s_Sh[si][sj] = 0.0;
        s_ST[si][sj] = 0.0;
    ...
    // Load halo cells (top, bottom, left, right + corners)
    // Top halo (ti == 0)
    if (ti == 0) {
        int gi_top = gi - 1;
        if (gi_top >= 0 && gj < c) {
            s_Sz[si - 1][sj] = GET(Sz, c, gi_top, gj);
            s_Sh[si - 1][sj] = GET(Sh, c, gi_top, gj);
            s_ST[si - 1][sj] = GET(ST, c, gi_top, gj);
        } else {
            s_Sz[si - 1][sj] = s_Sz[si][sj];
            s_Sh[si - 1][sj] = 0.0;
            s_ST[si - 1][sj] = 0.0;
        }
    }
    
    // Bottom halo (ti == TILE_SIZE-1)
    if (ti == TILE_SIZE - 1) {
        int gi_bot = gi + 1;
        if (gi_bot < r && gj < c) {
            s_Sz[si + 1][sj] = GET(Sz, c, gi_bot, gj);
            s_Sh[si + 1][sj] = GET(Sh, c, gi_bot, gj);
            s_ST[si + 1][sj] = GET(ST, c, gi_bot, gj);
        } else {
            s_Sz[si + 1][sj] = s_Sz[si][sj];
            s_Sh[si + 1][sj] = 0.0;
            s_ST[si + 1][sj] = 0.0;
    ...
    if (tj == 0) {
        int gj_left = gj - 1;
        if (gi < r && gj_left >= 0) {
            s_Sz[si][sj - 1] = GET(Sz, c, gi, gj_left);
            s_Sh[si][sj - 1] = GET(Sh, c, gi, gj_left);
            s_ST[si][sj - 1] = GET(ST, c, gi, gj_left);
        } else {
            s_Sz[si][sj - 1] = s_Sz[si][sj];
            s_Sh[si][sj - 1] = 0.0;
            s_ST[si][sj - 1] = 0.0;
        }
    }
    
    // Right halo (tj == TILE_SIZE-1)
    if (tj == TILE_SIZE - 1) {
        int gj_right = gj + 1;
        if (gi < r && gj_right < c) {
            s_Sz[si][sj + 1] = GET(Sz, c, gi, gj_right);
            s_Sh[si][sj + 1] = GET(Sh, c, gi, gj_right);
            s_ST[si][sj + 1] = GET(ST, c, gi, gj_right);
        } else {
            s_Sz[si][sj + 1] = s_Sz[si][sj];
            s_Sh[si][sj + 1] = 0.0;
            s_ST[si][sj + 1] = 0.0;
    ...
    // Corner halos (only 4 threads load corners)
    // Top-left corner
    if (ti == 0 && tj == 0) {
        int gi_tl = gi - 1;
        int gj_tl = gj - 1;
        if (gi_tl >= 0 && gj_tl >= 0) {
            s_Sz[si - 1][sj - 1] = GET(Sz, c, gi_tl, gj_tl);
            s_Sh[si - 1][sj - 1] = GET(Sh, c, gi_tl, gj_tl);
            s_ST[si - 1][sj - 1] = GET(ST, c, gi_tl, gj_tl);
        } else {
            s_Sz[si - 1][sj - 1] = s_Sz[si][sj];
            s_Sh[si - 1][sj - 1] = 0.0;
            s_ST[si - 1][sj - 1] = 0.0;
        }
    }
    ...
    // Top-right corner
    if (ti == 0 && tj == TILE_SIZE - 1) {
        int gi_tr = gi - 1;
        int gj_tr = gj + 1;
        if (gi_tr >= 0 && gj_tr < c) {
            s_Sz[si - 1][sj + 1] = GET(Sz, c, gi_tr, gj_tr);
            s_Sh[si - 1][sj + 1] = GET(Sh, c, gi_tr, gj_tr);
            s_ST[si - 1][sj + 1] = GET(ST, c, gi_tr, gj_tr);
        } else {
            s_Sz[si - 1][sj + 1] = s_Sz[si][sj];
            s_Sh[si - 1][sj + 1] = 0.0;
            s_ST[si - 1][sj + 1] = 0.0;
    ...
    // Bottom-left corner
    if (ti == TILE_SIZE - 1 && tj == 0) {
        int gi_bl = gi + 1;
        int gj_bl = gj - 1;
        if (gi_bl < r && gj_bl >= 0) {
            s_Sz[si + 1][sj - 1] = GET(Sz, c, gi_bl, gj_bl);
            s_Sh[si + 1][sj - 1] = GET(Sh, c, gi_bl, gj_bl);
            s_ST[si + 1][sj - 1] = GET(ST, c, gi_bl, gj_bl);
        } else {
            s_Sz[si + 1][sj - 1] = s_Sz[si][sj];
            s_Sh[si + 1][sj - 1] = 0.0;
            s_ST[si + 1][sj - 1] = 0.0;
    ...
    // Bottom-right corner
    if (ti == TILE_SIZE - 1 && tj == TILE_SIZE - 1) {
        int gi_br = gi + 1;
        int gj_br = gj + 1;
        if (gi_br < r && gj_br < c) {
            s_Sz[si + 1][sj + 1] = GET(Sz, c, gi_br, gj_br);
            s_Sh[si + 1][sj + 1] = GET(Sh, c, gi_br, gj_br);
            s_ST[si + 1][sj + 1] = GET(ST, c, gi_br, gj_br);
        } else {
            s_Sz[si + 1][sj + 1] = s_Sz[si][sj];
            s_Sh[si + 1][sj + 1] = 0.0;
            s_ST[si + 1][sj + 1] = 0.0;
    ...
    // Boundary check
    if (gi >= r || gj >= c) return;
    if (s_Sh[si][sj] <= 0) return;
    
    // Computation - now ALL neighbor accesses use shared memory!
    ...
    for (int k = 0; k < MOORE_NEIGHBORS; k++) {
        ...
        // ALL reads from shared memory now! (no global reads)
        sz = s_Sz[nsi][nsj];
        h[k] = s_Sh[nsi][nsj];
    ...
    // Rest of computation (same as V1/V2)
    H[0] = z[0];
    theta[0] = 0;
    eliminated[0] = false;
...
// VERSION 3: TILED massBalance with HALO CELLS
__global__ void massBalance_halo_kernel(
    int r, int c,
    int *Xi, int *Xj,
    double *Sh, double *Sh_next,
    ...
    double *Mf)
{
    // Shared memory with halo for ST
    __shared__ double s_ST[SHARED_SIZE][SHARED_SIZE];
    
    int gi = blockIdx.y * TILE_SIZE + threadIdx.y;
    int gj = blockIdx.x * TILE_SIZE + threadIdx.x;
    int si = threadIdx.y + HALO_SIZE;
    int sj = threadIdx.x + HALO_SIZE;
    int ti = threadIdx.y;
    int tj = threadIdx.x;
    ...
    // Load halo (simplified - just edges)
    if (ti == 0 && gi > 0 && gj < c) {
        s_ST[si - 1][sj] = GET(ST, c, gi - 1, gj);
    }
    if (ti == TILE_SIZE - 1 && gi + 1 < r && gj < c) {
        s_ST[si + 1][sj] = GET(ST, c, gi + 1, gj);
    }
    if (tj == 0 && gi < r && gj > 0) {
        s_ST[si][sj - 1] = GET(ST, c, gi, gj - 1);
    }
    if (tj == TILE_SIZE - 1 && gi < r && gj + 1 < c) {
        s_ST[si][sj + 1] = GET(ST, c, gi, gj + 1);
    }
    
    // Load corners
    if (ti == 0 && tj == 0 && gi > 0 && gj > 0) {
        s_ST[si - 1][sj - 1] = GET(ST, c, gi - 1, gj - 1);
    }
    if (ti == 0 && tj == TILE_SIZE - 1 && gi > 0 && gj + 1 < c) {
        s_ST[si - 1][sj + 1] = GET(ST, c, gi - 1, gj + 1);
    }
    if (ti == TILE_SIZE - 1 && tj == 0 && gi + 1 < r && gj > 0) {
        s_ST[si + 1][sj - 1] = GET(ST, c, gi + 1, gj - 1);
    }
    if (ti == TILE_SIZE - 1 && tj == TILE_SIZE - 1 && gi + 1 < r && gj + 1 < c) {
        s_ST[si + 1][sj + 1] = GET(ST, c, gi + 1, gj + 1);
    }
    ...
    for (int n = 1; n < MOORE_NEIGHBORS; n++) {
        ...
        // ALL reads from shared memory!
        neigh_t = s_ST[nsi][nsj];
...
void computeOutflows_halo_cuda(Sciara *sciara, dim3 grid, dim3 block)
{
    int *d_Xi, *d_Xj;
    ...
    computeOutflows_halo_kernel<<<grid, block>>>(
        sciara->domain->rows,
        sciara->domain->cols,
        d_Xi, d_Xj,
        sciara->substates->Sz,
...
void massBalance_halo_cuda(Sciara *sciara, dim3 grid, dim3 block)
{
    int *d_Xi, *d_Xj;
    cudaMalloc(&d_Xi, sizeof(int) * MOORE_NEIGHBORS);
    cudaMalloc(&d_Xj, sizeof(int) * MOORE_NEIGHBORS);
    ...
    massBalance_halo_kernel<<<grid, block>>>(
        sciara->domain->rows,
        sciara->domain->cols,
        d_Xi, d_Xj,
        sciara->substates->Sh,
...
int main(int argc, char **argv)
{
           ...
           (total_current_lava == -1 || total_current_lava > thickness_threshold))
    {
        ...
        // Apply the HALO computeOutflows kernel
        computeOutflows_halo_cuda(sciara, grid, block);

        // Apply the HALO massBalance kernel
        massBalance_halo_cuda(sciara, grid, block);
        cudaMemcpy(sciara->substates->Sh, sciara->substates->Sh_next, 
                   sizeof(double) * sciara->domain->rows * sciara->domain->cols, 
                   cudaMemcpyDeviceToDevice);
        cudaMemcpy(sciara->substates->ST, sciara->substates->ST_next, 
